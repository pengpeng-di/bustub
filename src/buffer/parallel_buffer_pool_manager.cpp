...
size_t ParallelBufferPoolManager::GetPoolSize() {
  ...
  int ssize = 0;
  for (int i = 0; i < num_instances_; i++) {
    ssize += instances_[i]->GetPoolSize();
  }
  return ssize;
}
...
Page *ParallelBufferPoolManager::NewPgImp(page_id_t *page_id) {
  ...
  int start = static_cast<int>(page_instance_index_++ % num_instances_);
  int idx = start;
  Page *res = nullptr;
  while (true) {
    res = instances_[idx]->NewPage(page_id);
    if (res != nullptr) {
      return res;
    }
    idx = static_cast<int>((idx + 1) % num_instances_);
    if (idx == start) {
      return nullptr;
    }
  }
